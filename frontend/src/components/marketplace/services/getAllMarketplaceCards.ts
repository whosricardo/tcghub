...
export async function getAllMarketplaceCards(param: CardSearchParams, page: number , limit: number):Promise<CardResponse<CardProps>>{
    const apiPage  = page - 1;
    const searchParams = buildSearch(param).toString();
    const url_base = `/single-cards?page=${apiPage}&size=${limit}`
